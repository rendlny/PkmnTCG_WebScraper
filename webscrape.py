...
lastPageUrl = Data.URL_START + numOfPagesTxt + urlEnd
headers = {
    'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) '
                  'Chrome/39.0.2171.95 Safari/537.36',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'}
...
while count <= numOfPages:
    url = Data.URL_START + str(count) + urlEnd
    webpage = urllib.request.urlopen(url)
    page = BeautifulSoup(webpage, "html.parser")
    ...
    for cards in cardsSection.findAll('a'):
        ...
        # cards
        url = Data.URL_START + str(setCode) + "-series/" + str(setCode) + str(setNum) + "/" + str(cardNum) + "/"
        print(url)
        page = scrapeUrl(url)
        # GETTING CARD NAME ###########################################
        ...
        # Trainer Cards, checking trainer type
            ...
            if 'Item' in stage:
                ...
                print("Trainer - Tool")
                rule = "You may play as many Item cards as you like during your turn (before your attack)"
                specialRule = "Attach a Pokemon Tool to 1 of your Pokemon that doesn't already have a Pokemon Tool " \
                              "attached to it."

            elif 'Stadium' in stage:
                print("Trainer - Stadium")
                rule = "This card stays in play when you play it. Discard this card if another Stadium card comes " \
                       "into play. If another card with the same name is in play, you can't play this card."
